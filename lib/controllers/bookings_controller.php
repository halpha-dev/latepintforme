...
if ( ! class_exists( 'OsBookingsController' ) ) :
	...
	class OsBookingsController extends OsController {
        ...
        if(!empty($filter['created_date_from']) && !empty($filter['created_date_to'])){
            $oo = new OsWpDateTime($filter['created_date_from']);
            if($oo->format('Y')<1600)
            {
                $md = JalaliCalendar::toGregorianDate($oo->format('Y'),$oo->format   ('m'),$oo->format('d'));
                $filter['created_date_from'] = $md->format('Y-m-d');
        ...
        if(!empty($filter['booking_date_from']) && !empty($filter['booking_date_to'])){
            $oo = new OsWpDateTime($filter['booking_date_from']);
            if($oo->format('Y')<1600)
            {
                $md = JalaliCalendar::toGregorianDate($oo->format('Y'),$oo->format   ('m'),$oo->format('d'));
                $filter['booking_date_from'] = $md->format('Y-m-d');
        
             }
            $oo = new OsWpDateTime($filter['booking_date_to']);
            if($oo->format('Y')<1600)
            {
                $md = JalaliCalendar::toGregorianDate($oo->format('Y'),$oo->format   ('m'),$oo->format('d'));
                $filter['booking_date_to'] = $md->format('Y-m-d');
      ...
      if($calendar_start_date->format('Y')<1600)
      {
          $md = JalaliCalendar::toGregorianDate($calendar_start_date->format('Y'),$calendar_start_date->format('m'),$calendar_start_date->format('d'));
        $calendar_start_date = new OsWpDateTime($md->format('Y-m-d'));
      }
