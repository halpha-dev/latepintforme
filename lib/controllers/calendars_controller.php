	...
	class OsCalendarsController extends OsController {
		...
		public function apply_period_block() {
			...
			try {
				...
				if ( $blocked_period_settings['full_day_off'] == 'yes' ) {
					...
					$work_period_obj->week_day    = $day_date->jformat( 'N' );
		...
		public function view() {
			...
			if ( $calendar_settings['view'] == 'list' ) {
				...
				$top_date_label = OsUtilHelper::translate_jmonths( $oo->jdate('F', strtotime($calendar_start->format('Y-m-d'))), false );
			} else {
				// get calendar start date as object [Stuxan delete this line]
				$target_date = new OsWpDateTime($calendar_settings['target_date_string']);
				switch ( $calendar_settings['view'] ) {
					case 'day':
						...
						$top_date_label   = OsUtilHelper::translate_jmonths( $oo->jdate('F j', strtotime($calendar_start->format('Y-m-d'))), false );
						break;
					case 'week':
						$calendar_start   = ( clone $target_date )->modify( $target_date->format('w') == 6 ? 'this saturday' : 'last saturday' );
						$calendar_end     = ( clone $calendar_start )->modify('+6 days');
						$top_date_label   = OsUtilHelper::translate_jmonths( $oo->jdate('F', strtotime($calendar_start->format('Y-m-d'))), false ) . ' ' . $oo->jdate('j', strtotime($calendar_start->format('Y-m-d'))) . ' - ' . OsUtilHelper::translate_jmonths( $oo->jdate('F', strtotime($calendar_end->format('Y-m-d'))), false ) . ' ' . $oo->jdate('j', strtotime($calendar_end->format('Y-m-d')));
						...
						break;
					case 'month':
						$calendar_start   = explode( '-', $oo->jdate('Y-m-01', strtotime($calendar_settings['target_date_string'])));
						$calendar_start   = new OsWpDateTime($oo->jalali_to_gregorian( $calendar_start[0], $calendar_start[1], $calendar_start[2], '-'));
						$calendar_end 	  = explode( '-', $oo->jdate('Y-m-t', strtotime($calendar_settings['target_date_string'])));
						$calendar_end 	  = new OsWpDateTime($oo->jalali_to_gregorian( $calendar_end[0], $calendar_end[1], $calendar_end[2], '-'));
						$top_date_label   = OsUtilHelper::translate_jmonths( $oo->jdate('F', strtotime($calendar_start->format('Y-m-d'))), false );
						$prev_target_date = explode( '-', $oo->jdate('Y-m-01', strtotime('-1 MONTH', strtotime($calendar_settings['target_date_string']))));
						$prev_target_date = new OsWpDateTime($oo->jalali_to_gregorian( $prev_target_date[0], $prev_target_date[1], $prev_target_date[2], '-'));
						$next_target_date = explode( '-', $oo->jdate('Y-m-01', strtotime('+1 MONTH', strtotime($calendar_settings['target_date_string']))));
						$next_target_date = new OsWpDateTime($oo->jalali_to_gregorian( $next_target_date[0], $next_target_date[1], $next_target_date[2], '-'));
						break;
				}
			...
			$top_date_year = ! empty( $calendar_start ) ? $calendar_start->jformat( 'Y' ) : '';
