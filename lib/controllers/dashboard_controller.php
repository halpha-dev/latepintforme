...
if ( ! class_exists( 'OsDashboardController' ) ) :
  ...
  class OsDashboardController extends OsController {
    ...
    public function widget_daily_bookings_chart($date_from = false, $date_to = false){
      ...
      if($date_from->format('Y')<1600)
      {
        $gg = JalaliCalendar::toGregorian($date_from->format('Y'),$date_from->format('m'),$date_from->format('d'));
        $date_from = OsWpDateTime::os_createFromFormat('Y-m-d', $gg[0].'-'.$gg[1].'-'.$gg[2]);
      }
      if($date_to->format('Y')<1600)
      {
        $gg = JalaliCalendar::toGregorian($date_to->format('Y'),$date_to->format('m'),$date_to->format('d'));
        $date_to = OsWpDateTime::os_createFromFormat('Y-m-d', $gg[0].'-'.$gg[1].'-'.$gg[2]);
      }
      ...
      for($day_date=clone $date_from; $day_date<=$date_to; $day_date->modify('+1 day')){
        $daily_chart_data[OsTimeHelper::get_nice_jdate_with_optional_year($day_date->format('Y-m-d'), false, true)] = 0;
      }
      // update the days with count of bookings
      foreach($daily_bookings as $bookings_for_day){
        $daily_chart_data[OsTimeHelper::get_nice_jdate_with_optional_year(gmdate( 'Y-m-d', strtotime($bookings_for_day->start_date)), false, true)] = $bookings_for_day->bookings_per_day;
      }
    ...
    public function widget_bookings_and_availability_timeline(){
      ...
      if(isset($this->params['date_from']) && $target_date->format('Y') < 1600) {
        $gg = JalaliCalendar::toGregorian($target_date->format('Y'),$target_date->format('m'),$target_date->format('d'));
        $target_date = OsWpDateTime::os_createFromFormat('Y-m-d', $gg[0].'-'.$gg[1].'-'.$gg[2]);
      }
      $services = new OsServiceModel();
      $agents = new OsAgentModel();
      $locations = new OsLocationModel();
