...
if ( ! class_exists( 'OsProcessJobsController' ) ) :
	...
	class OsProcessJobsController extends OsController {
		...
		public function view_job_run_result() {
			...
			$this->vars['job']          = $job;
			$run_datetime = explode(" ", $data->run_datetime_utc);
			$run_date = new OsWpDateTime(OsWpDateTime::os_createFromFormat("Y-m-d", $run_datetime[0])->format('Y-m-d'));
			$data->run_datetime_utc = $run_date->jformat(OsSettingsHelper::get_readable_date_format()) . " ". $run_datetime[1];
			...
			if ( ! empty( $result_data['ran_actions_info'] ) ) {
				foreach ( $result_data['ran_actions_info'] as $ran_action_info ) {
					if ( $ran_action_info['id'] == $action->id ) {
						$run_datetime = explode(" ", $ran_action_info['run_datetime_utc']);
						$run_date = new OsWpDateTime(OsWpDateTime::os_createFromFormat("Y-m-d", $run_datetime[0])->format('Y-m-d'));
						$ran_action_info['run_datetime_utc'] = $run_date->jformat(OsSettingsHelper::get_readable_date_format()) . " ". $run_datetime[1];
