...
if ( ! class_exists( 'OsSettingsController' ) ) :
	...
	class OsSettingsController extends OsController {
		...
		public function save_custom_day_schedule() {
			...
			$status        = LATEPOINT_STATUS_SUCCESS;
			$day_date      = OsWpDateTime::backend_date_time( $this->params['start_custom_date'] );
			// if end date is provided and is range
			$period_type = ( $this->params['period_type'] == 'range' && $this->params['end_custom_date'] ) ? 'range' : 'single';

			$start_date                = OsWpDateTime::backend_date_time( $this->params['start_custom_date'] );
			$end_date                  = ( $period_type == 'range' ) ? OsWpDateTime::backend_date_time( $this->params['end_custom_date'] ) : $start_date;
			...
			for ( $day_date = clone $start_date; $day_date <= $end_date; $day_date->modify( '+1 day' ) ) {
				...
				foreach ( $work_periods as &$work_period ) {
					...
					$work_period['week_day']    = $day_date->jformat( 'N' );
		...
		public function custom_day_schedule_form() {
			...
			$this->vars['target_date']         = OsWpDateTime::backend_date_time( $target_date_string );
			...
			if ( $chain_id ) {
				...
				if ( $chained_work_period ) {
					$this->vars['chain_end_date'] = OsWpDateTime::backend_date_time( $chained_work_period->custom_date );
				} else {
