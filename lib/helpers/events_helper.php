...
class OsEventsHelper {
	...
	public static function week_grid(OsWpDateTime $calendar_start, OsWpDateTime $calendar_end, OsWpDateTime $target_date, array $services_with_resources, array $filter = []){
		...
		for ($day_date = clone $calendar_start; $day_date <= $calendar_end; $day_date->modify('+1 day')) {
			...
			$html.= '<div class="le-day-weekday">'.OsBookingHelper::get_weekday_name_by_number($day_date->jformat('N')).'</div>';
			if ($day_date->jformat('d') == '1') $html.= '<div class="os-day-month">' . OsUtilHelper::get_jmonth_name_by_number($day_date->jformat('n')) . '</div>';
			$html.= '<div class="le-day-number" 
					...
					data-os-action="' . OsRouterHelper::build_route_name('events', 'events_day_view') . '">' . $day_date->jformat('j') .
				'</div>';
			$html.= '</div>';
	...
	public static function month_grid(OsWpDateTime $calendar_start, OsWpDateTime $calendar_end, OsWpDateTime $target_date, array $services_with_resources, array $filter = []){
		...
		$weekdays = OsBookingHelper::get_jweekdays_arr();
		...
		for ($day_date = clone $calendar_start; $day_date <= $calendar_end; $day_date->modify('+1 day')) {
			$is_today = ($day_date->format('Y-m-d') == $now_datetime->format('Y-m-d'));
			$is_day_in_past = ($day_date->format('Y-m-d') < $now_datetime->format('Y-m-d'));
			$is_target_month = ($day_date->jformat('m') == $target_date->jformat('m'));
			$is_next_month = ($day_date->jformat('m') > $target_date->jformat('m'));
			$is_prev_month = ($day_date->jformat('m') < $target_date->jformat('m'));
	...
	public static function events_grid(OsWpDateTime $target_date, array $filter = [], $range_type = 'week', array $restrictions = []): string {
		...
		$html.= OsFormHelper::hidden_field('restrictions', wp_json_encode($restrictions));
			...
			if($range_type == 'week'){
				if($calendar_start->jformat('M') == $calendar_end->jformat('M')){
					$html.= '<div class="le-week">' . $calendar_start->jformat('M j') . ' - ' . $calendar_end->jformat('j') .'</div>';
				}else{
					$html.= '<div class="le-week">' . $calendar_start->jformat('M j') . ' - ' . $calendar_end->jformat('M j') .'</div>';
				}
			}
			if($range_type == 'month'){
				$html.= '<div class="le-month">' . OsUtilHelper::get_jmonth_name_by_number($target_date->jformat('n')) . '</div>';
			}
