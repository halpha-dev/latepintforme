...
class OsTimeHelper {
  ...
  public static function reformat_date_string( $date_string, $from_format, $to_format ) {
	if( ! empty($date_string) )
	{
		...
		$oj = $oo->getJalaliSQLDate($date_string);
		if($oj[0]<1800)
		{
			$md = JalaliCalendar::toGregorianDate($oj[0], $oj[1], $oj[2]);
			$date_string = $md->format($from_format);
		}
	}
  ...
  public static function reformat_jdate_string( $date_string, $from_format, $to_format ) {
    $start_date_obj = OsWpDateTime::os_createFromFormat( $from_format, $date_string );
    $oo = new OsWpDateTime($start_date_obj->format('Y-m-d'));
    return $oo->jformat( $to_format );
  }
  ...
  public static function nice_jdate( $date ) {
		if ( $date == OsTimeHelper::today_date( 'Y-m-d' ) ) {
			$nice_date = __( 'Today', 'latepoint' );
		} else {
			$nice_date = self::get_nice_jdate_with_optional_year( $date, true );
		}
  ...
  public static function jdate_from_db( $date_string, $format = false, string $output_timezone_name = 'UTC' ) {
    $timezone = new DateTimeZone( 'UTC' );
		$date_obj = OsWpDateTime::os_createFromFormat( LATEPOINT_DATETIME_DB_FORMAT, $date_string, $timezone );
    $date_obj->setTimezone( new DateTimeZone( $output_timezone_name ) );
    $md = JalaliCalendar::date($format, strtotime($date_obj->format('Y-m-d H:i:s')));
    if($format) return $md;
    return $date_obj;
  }
  ...
  public static function today_jdate($date_format = 'Y-m-d'){

    return JalaliCalendar::date($date_format);
  }
  ...
  public static function now_jdatetime_in_format( $date_format = LATEPOINT_DATETIME_DB_FORMAT, string $timezone = 'UTC' ) {
		$now = self::now_datetime_object();
		$now->setTimezone( new DateTimeZone( "UTC" ) );

    return JalaliCalendar::date($date_format, strtotime($now->format('Y-m-d H:i:s')));
	}
...
public static function format_date_with_locale( string $format, DateTime $date_obj ): string {
	$oo = new OsWpDateTime( $date_obj->format('Y-m-d') );
	return OsUtilHelper::translate_jmonths( $oo->jformat($format) );
}
...
public static function get_nice_date_with_optional_year( $date, $show_year_if_not_current = true, $short_month = false ) {
	...
	if ( ! $show_year_if_not_current || ( $d->format( 'Y' ) == OsTimeHelper::today_date( 'Y' ) ) ) {
		return OsUtilHelper::translate_months(
			$d->format( OsSettingsHelper::get_readable_date_format( true, $short_month ) )
		);
	} else {
		return OsUtilHelper::translate_months(
			$d->format( OsSettingsHelper::get_readable_date_format( false, $short_month ) )
		);
	}
}

public static function get_nice_jdate_with_optional_year( $date, $show_year_if_not_current = true ) {
	$d = OsWpDateTime::os_createFromFormat("Y-m-d", $date);
	if ( ! $d ) {
		return $date;
	}
	$oo = new OsWpDateTime($d->format('Y-m-d'));
	if ( ! $show_year_if_not_current || ( $d->format( 'Y' ) == OsTimeHelper::today_date( 'Y' ) ) ) {
		return OsUtilHelper::translate_jmonths( $oo->jformat( OsSettingsHelper::get_readable_date_format(true) ) );
	} else {
		return OsUtilHelper::translate_jmonths( $oo->jformat( OsSettingsHelper::get_readable_date_format() ) );
	}
}
...
public static function get_readable_date( DateTime $date, string $output_timezone_name = '' ): string {
    ...
    if ( $hours == '12' && ($ampm == 'am' || $ampm == 'ق.ظ') ) {
      // midnight
      $hours = '0';
    }
    if ( ( $ampm == 'pm' || $ampm == 'ب.ظ') && $hours < 12 ) {
  ...
  public static function format_to_nice_jdate( $datetime, $hide_year_if_current = false ): string {
		if ( $datetime instanceof DateTime ) {
      		$oo = new OsWpDateTime($datetime->format('Y-m-d H:i:s'));
			if ( $hide_year_if_current && ( $oo->jformat( 'Y' ) == self::today_jdate( 'Y' ) ) ) {
				$format = OsSettingsHelper::get_readable_date_format( true );
			} else {
				$format = OsSettingsHelper::get_readable_date_format();
			}
