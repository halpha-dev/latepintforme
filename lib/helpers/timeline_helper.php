  ...
  public static function availability_timeline_off(string $off_label = ''){
    ...
    if($booking_slot->can_accomodate($booking_request->total_attendees)){
      ...
      if($settings['book_on_click']){
        ...
        $data_attrs = 'data-date="'.$booking_slot->start_date.'" data-formatted-date="'.OsTimeHelper::reformat_jdate_string($booking_slot->start_date, 'Y-m-d', OsSettingsHelper::get_date_format()).'" data-minutes="'.$booking_slot->start_time.'"';
		...
		$timeslot_html = '<div '.$data_attrs.' style="'.$style.'" class="agent-timeslot '.$timeslot_class.'">
												<span class="agent-timeslot-label"><div class="at-sub-value">'.OsTimeHelper::get_nice_jdate_with_optional_year($booking_slot->start_date).'</div><div class="at-main-value">'.OsTimeHelper::minutes_to_hours_and_minutes($booking_slot->start_time).'</div></span>'.
						            $tick_html.'
											</div>';
