...
class OsWorkPeriodsHelper {
  ...
  public static function get_work_periods(\LatePoint\Misc\Filter $filter, bool $as_models = false): array{
    ...
    $weekdayinjalali=[1=>3,2=>4,3=>5,4=>6,5=>7,6=>1,7=>2];
    ...
    $query_args = array();
     ...
     if($filter->week_day){
	     ...
	     if(!$filter->date_to || ($filter->date_from == $filter->date_to)){
				 // single date
				 $query_args['week_day'] = $weekdayinjalali[$date_from_obj->format('N')];
	     }else{
				 // date range
		     ...
		     // otherwise loop through them and find which weekdays we need to query for
				 if($date_to_obj->diff($date_from_obj)->format("%a") < 6){
					 for($day_date=clone $date_from_obj; $day_date<=$date_to_obj; $day_date->modify('+1 day')){
						 $query_args['week_day'][] = $weekdayinjalali[$day_date->format('N')];
					 }
				 }
	     }
     }
  ...
  public static function generate_days_with_custom_schedule($args = array()){
    ...
    if($work_periods && isset($work_periods[0])){
      ...
      if($date->format('Y') != gmdate('Y')) $html.=  '<div class="os-form-sub-header sub-level"><h3>'.esc_html($date->jformat('Y')).'</h3></div>';
    }
    $chained_periods = [];
      ...
      if($work_periods){
        ...
        foreach($work_periods as $work_period){
          ...
          if($chained_periods){
            ...
            foreach($chained_periods as $chain_id => $chained_period){
              ...
              if($processing_year != $range_start_date->format('Y')){
                $html.= '</div><div class="os-form-sub-header sub-level"><h3>'.esc_html($range_start_date->jformat('Y')).'</h3></div><div class="custom-day-work-periods">';
              ...
              $html.= '<div class="custom-day-number">'.esc_html($range_start_date->jformat('d').' - '.$range_end_date->jformat('d')) .'</div>';
              if($range_start_date->jformat('n') != $range_end_date->jformat('n')){
                $html.= '<div class="custom-day-month">'.esc_html(OsUtilHelper::get_jmonth_name_by_number($range_start_date->jformat('n')).'-'.OsUtilHelper::get_jmonth_name_by_number($range_end_date->jformat('n'))).'</div>';
              }else{
                $html.= '<div class="custom-day-month">'.esc_html(OsUtilHelper::get_jmonth_name_by_number($range_start_date->jformat('n'))).'</div>';
              }
              $html.= '</div>';
          ...
          if(empty($work_period->chain_id)){
            ...
            if($processing_year != $date->format('Y')) $html.= '</div><div class="os-form-sub-header sub-level"><h3>'.esc_html($date->jformat('Y')).'</h3></div><div class="custom-day-work-periods">';
            ...
            $html.= '<div class="custom-day-work-period-i">';
            $html.= '<div class="custom-day-number">'.esc_html($date->jformat('d')).'</div>';
            $html.= '<div class="custom-day-month">'.esc_html(OsUtilHelper::get_jmonth_name_by_number($date->jformat('n'))).'</div>';
            $html.= '</div>';
            $work_periods_for_date_model = new OsWorkPeriodModel();
  ...
  public static function generate_work_periods(array $work_periods, \LatePoint\Misc\Filter $filter, bool $is_new_record = false){
    ...
    $weekdayind=[1=>3,2=>4,3=>5,4=>6,5=>7,6=>1,7=>2];
    for($i=1; $i<=7; $i++){
      $is_day_off = true;
      $period_forms_html = '';
      ...
      <div class="weekday-schedule-w <?php echo $is_day_off ? 'day-off' : ''; ?>">
        <div class="ws-head-w">
          ...
          <div class="ws-head">
            <div class="ws-day-name"><?php echo esc_html(OsBookingHelper::get_jweekday_name_by_number($i, true)); ?></div>
        ...
        <div class="weekday-schedule-form">
          ...
          <div class="ws-period-add"
            ...
            <div class="add-period-label">
                ...
                echo esc_html(sprintf(__('Add another work period for %s', 'latepoint'), OsBookingHelper::get_jweekday_name_by_number($i, true))); ?>
            </div>
          </div>
        </div>
      </div>
