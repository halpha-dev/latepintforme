...
class OsWpDateTime extends DateTime {
  function __construct(?string $time = 'now', ?DateTimeZone $timezone = null){
	...
	public static function backend_date_time( $date_string = '', $format = 'Y-m-d' ) {
		if( ! empty($date_string) )
		{
			$oo = new JalaliCalendar();
			$oj = $oo->getJalaliSQLDate($date_string, false);
			if($oj && $oj[0]<1800)
			{
				$md = JalaliCalendar::toGregorianDate($oj[0], $oj[1], $oj[2]);
				$date_string = $md->format( $format );
			}
		}
	...
	public static function date_from_db_jformat($date_string, $default = ''){
		if(empty($date_string)) return $default;
		try{
			$date = self::os_createFromFormat('Y-m-d', $date_string);
			return JalaliCalendar::date(OsSettingsHelper::get_date_format(), strtotime($date->format('Y-m-d')));
		}catch(Exception $e){
			return $default;
		}
	  }
  ...
  public static function os_createFromFormat($format, $datetime_string, $timezone = false){
	$oo = new OsWpDateTime($datetime_string);
	if($oo->format('Y')<1600)
	{
		$md = JalaliCalendar::toGregorianDate($oo->format('Y'),$oo->format('m'),$oo->format('d'));
		$datetime_string = $md->format($format);
	}
  ...
  public function jformat($format) {
	//   return $this->format($format);
	  return JalaliCalendar::date($format,strtotime(self::format('Y-m-d')));
  }
  public function jfirstdaymonth($gregorian = true)
  {
	$jalaliinfs = [$this->format('Y'), $this->format('m'), $this->format('d')];
	  if($gregorian)
	  {
		$jalaliinfs = JalaliCalendar::toJalali($jalaliinfs[0],$jalaliinfs[1],$jalaliinfs[2]);
	  }
	  
	  $md =  JalaliCalendar::toGregorianDate($jalaliinfs[0],$jalaliinfs[1],1);
	  return $md->format('Y-m-d');
  }
  public function jlastdaymonth($gregorian = true)
  {
	$jalaliinfs = [$this->format('Y'), $this->format('m'), $this->format('d')];
	if($gregorian)
	{
	...
	$md = JalaliCalendar::toGregorianDate($jalaliinfs[0],$jalaliinfs[1],JalaliCalendar::jalaliMonthLength($jalaliinfs[0],$jalaliinfs[1]));
	return $md->format('Y-m-d');
  }
}