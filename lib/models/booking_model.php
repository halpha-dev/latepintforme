...
class OsBookingModel extends OsModel {
	...
	public function get_nice_jcreated_at( $include_time = true ) {
		$format = $include_time ? OsSettingsHelper::get_readable_date_format() . ' ' . OsSettingsHelper::get_readable_time_format() : OsSettingsHelper::get_readable_date_format();
		$utc_date = date_create_from_format( LATEPOINT_DATETIME_DB_FORMAT, $this->created_at );
		$wp_timezone_date = $utc_date->setTimezone(OsTimeHelper::get_wp_timezone());
		...
		return $oo->jdate( $format, strtotime( $wp_timezone_date->format('Y-m-d H:i:s') ) );
	}
		...
		if ( $this->service_id && $this->agent_id &&  $customer_requirement_satisfied) {
			...
			if ( $this->agent_id == LATEPOINT_ANY_AGENT && $this->location_id == LATEPOINT_ANY_LOCATION ) {
				...
				if ( ! OsBookingHelper::is_booking_request_available( \LatePoint\Misc\BookingRequest::create_from_booking_model( $this ) ) ) {
					...
					$error_message = sprintf( __( 'Unfortunately the selected time slot "%1$s" for "%2$s" is not available anymore, please select another timeslot.', 'latepoint' ), $this->get_nice_start_jdatetime_for_customer(), $this->service->name );
	...
	public function get_nice_start_jdate_for_customer($customer_timezone = false, $hide_year = false) {
		if(!$customer_timezone) $customer_timezone = $this->get_customer_timezone();
		return OsUtilHelper::translate_jmonths($this->format_start_jdate_and_time( OsSettingsHelper::get_readable_date_format($hide_year), false, $customer_timezone ));
	}
	...
	public function get_nice_start_jdatetime_for_customer($customer_timezone = false) {
		if(!$customer_timezone) $customer_timezone = $this->get_customer_timezone();
		return OsUtilHelper::translate_jmonths($this->format_start_jdate_and_time( OsSettingsHelper::get_readable_datetime_format(), false, $customer_timezone ));
	}
	...
	public function get_nice_end_jdate( $hide_year_if_current = false ) {
		$datetime = OsWpDateTime::os_createFromFormat( "Y-m-d", $this->end_date );
		OsTimeHelper::format_to_nice_jdate($datetime, $hide_year_if_current);
	}
	...
	public function get_nice_start_jdate( $hide_year_if_current = false ) : string {
		$datetime = OsWpDateTime::os_createFromFormat( "Y-m-d", $this->start_date );
		return OsTimeHelper::format_to_nice_jdate($datetime, $hide_year_if_current);
	}
	...
	public function get_nice_start_jdatetime( bool $hide_if_today = true, bool $hide_year_if_current = true ): string {
		if ( $hide_if_today && $this->start_date == OsTimeHelper::today_date( 'Y-m-d' ) ) {
			$date = __( 'Today', 'latepoint' );
		} else {
			$date = $this->get_nice_start_jdate( $hide_year_if_current );
		}
	...
	public function get_nice_jdatetime_for_summary(string $viewer = 'customer'){
		$nice_datetime = '';
		if($this->start_date){
			$nice_datetime = $this->get_nice_start_jdatetime(false);
			if(OsSettingsHelper::is_on( 'show_booking_end_time') && !empty($this->end_time) && !empty($this->start_time)){
				$nice_datetime = $nice_datetime.' - '.$this->get_nice_end_time();
			}
		}
		 ...
		 * @returns {string} Filtered nice datetime
		 */
		$nice_datetime = apply_filters('latepoint_get_nice_jdatetime_for_summary', $nice_datetime, $this, $viewer);
		return $nice_datetime;
	}
	...
	public function format_start_jdate() {
		if ( empty( $this->start_date ) ) {
			$date             = new OsWpDateTime();
			$this->start_date = $date->format( 'Y-m-d' );
		} else {
		...
		return OsUtilHelper::translate_jmonths( JalaliCalendar::date(OsSettingsHelper::get_date_format(), strtotime($date->format('Y-m-d H:i:s'))) );
	}
	...
	public function format_start_jdate_and_time( $format = LATEPOINT_DATETIME_DB_FORMAT, $input_timezone = false, $output_timezone = false ) {
		if ( ! $input_timezone ) {
			$input_timezone = OsTimeHelper::get_wp_timezone();
		}
		if ( ! $output_timezone ) {
		...
		if ( is_null( $this->start_time ) || $this->start_time === '' ) {
			...
			$date = OsWpDateTime::os_createFromFormat( "Y-m-d", $this->start_date );
			if ( $date ) {
				return OsUtilHelper::translate_jmonths( JalaliCalendar::date(OsSettingsHelper::get_readable_date_format(), strtotime($date->format('Y-m-d H:i:s'))) );
			} else {
				return __( 'Invalid Date/Time', 'latepoint' );
			}
		} else {
			...
			if ( $date ) {
				...
				return OsUtilHelper::translate_jmonths( JalaliCalendar::date($format, strtotime($date->format('Y-m-d H:i:s'))) );
			} else {
				return __( 'Invalid Date/Time', 'latepoint' );
			}
		}
	...
	public function start_jdatetime_in_format( string $format, string $output_in_timezone_name ) : string {
		if(empty($this->start_datetime_utc)) return '';
		$booking_start_datetime = OsTimeHelper::date_from_db( $this->start_datetime_utc );
		$booking_start_datetime->setTimezone( new DateTimeZone($output_in_timezone_name) );
		return JalaliCalendar::date($format, strtotime($booking_start_datetime->format('Y-m-d H:i:s')));
	}
	...
	public function get_nice_start_jdate_in_timezone(string $timezone_name = '', $hide_year_if_current = false) : string{
		$datetime = $this->get_start_datetime($timezone_name);
		return OsTimeHelper::format_to_nice_jdate($datetime, $hide_year_if_current);
	}
	...
	public function get_nice_end_jdate_in_timezone(string $timezone_name = '', $hide_year_if_current = false) : string{
		$datetime = $this->get_end_datetime($timezone_name);
		return OsTimeHelper::format_to_nice_jdate($datetime, $hide_year_if_current);
	}
