...
if($bookings){
  foreach ($bookings as $booking): 
    $jcreated_at_time = explode(" ", $booking->created_at)[1];
		$jcreated_at = new OsWpDateTime($booking->created_at);
  ?>
    <tr class="os-clickable-row" <?php echo OsBookingHelper::quick_booking_btn_html($booking->id); ?>>
      ...
      <?php } ?>
      <td><strong><?php echo esc_html($booking->nice_start_jdate); ?></strong> <span class="os-dot"></span> <span><?php echo esc_html($booking->nice_start_time); ?></span></td>
      ...
      <td><?php echo esc_html( $jcreated_at->jformat('j F ,Y') . ' ' . $jcreated_at_time ); ?></td>
      <?php 
