						...
						<div class="timeline-and-availability-contents shows-<?php echo esc_attr($what_to_show); ?>">
							<div class="agent-day-bookings-timeline-compact-w">
								...
								<div class="agents-timelines-w">
									<?php if($show_today_indicator) echo '<div class="current-time-indicator" style="right: '.esc_attr($time_now_indicator_left_offset).'%"><span>'.esc_html($time_now_label).'</span></div>'; ?>
													...
													if($bookings && $work_total_minutes){
														...
														foreach($bookings as $index => $booking){
															...
															if(OsBookingHelper::check_if_group_bookings($booking, $next_booking)){
																...
																$right = ($booking->start_time - $work_boundaries->start_time) / $work_total_minutes * 100;
																if($width <= 0 || $right >= 100 || (($right + $width) <= 0)) continue;
																if($right < 0){
																	$width = $width + $right;
																	$right = 0;
																}
																if(($right + $width) > 100) $width = 100 - $right;
																...
																echo '<div data-start="'.esc_attr($booking->start_time).'" data-end="'.esc_attr($booking->end_time).'" class="booking-block status-'.esc_attr($booking->status).'" '.$action_html.' style="background-color: '.esc_attr($booking->service->bg_color).'; right: '.esc_attr($right).'%; width: '.esc_attr($width).'%;'.esc_attr($custom_height).'">';
