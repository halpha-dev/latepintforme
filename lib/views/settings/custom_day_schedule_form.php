  	...
  	<div class="custom-day-schedule-w">
	  	<div class="custom-day-calendar" data-show-schedule="<?php echo ($day_off) ? 'no' : 'yes'; ?>" data-period-type="<?php echo ($chain_id) ? 'range' : 'single'; ?>" data-picking="start">
				<div class="custom-day-settings-w">
					...
					<div class="start-day-input-w">
						<?php echo OsFormHelper::text_field('start_custom_date', false, ($date_is_preselected) ? $target_date->jformat('Y-m-d') : '', ['placeholder' => __('Pick a Start', 'latepoint'), 'theme' => 'simple']); ?>
					</div>
					<div class="end-day-input-w">
						<?php echo OsFormHelper::text_field('end_custom_date', false, ($chain_end_date) ? $chain_end_date->jformat('Y-m-d') : '', ['placeholder' => __('Pick an End', 'latepoint'), 'theme' => 'simple']); ?>
					</div>
				</div>
	  		<div class="custom-day-calendar-head">
	  			...
	  			<?php echo OsFormHelper::select_field('custom_day_calendar_month', false, OsUtilHelper::get_jmonths_for_select(), $target_date->jformat('n')); ?>
	  			<?php echo OsFormHelper::select_field('custom_day_calendar_year', false, [OsTimeHelper::today_jdate('Y'), OsTimeHelper::today_jdate('Y') + 1], $target_date->format('Y')); ?>
	  		</div>
	  	...
	  	<div class="custom-day-schedule">
		      ...
		      if($day_off){
		      	...
		      	$preselected_date = $target_date->jformat('Y-m-d');
